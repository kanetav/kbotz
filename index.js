...
ban = []
cr = '*KBOTZ*'
vz = '*Terima Kasih Sudah Daftar Di KBOTZ*'
const vcard = 'BEGIN:VCARD\n' //
            + 'VERSION:3.0\n' //
            + 'FN: Kanetav\n' //
            + 'ORG:Creator KBOTZ;\n' //
            + 'TEL;type=CELL;type=VOICE;waid=6288802377901:+6288802377901 \n' //
            + 'END:VCARD'
prefix = '!'
blocked = []
...
async function starts() {
	...
	client.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color(' POWERED BY Kanetav'))
	})
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `[ Kedatangan Member Baru Nih _*${mdata.subject}*_ ] \n*_____________*\n@${num.split('@')[0]}\nSelamat Bergabung Ya Kak\n *_____________*\nMoga Aja Betah Ya Kak Oya Intro Dulu Ya !`
				...
				teks = `Heyyy @${num.split('@')[0]} Kenapa Keluar Sih Ah Malesin Yodah Awas Aja Balik Lagi `
				let buffer = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buffer, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[❗] Perintah ini hanya bisa di gunakan untuk user mod KBOTZ ❌',
					...
					userB: `──「 DAFTAR 」──\nMaaf sepertinya kamu belum terdaftar di KBOTZ\nDaftar untuk menikmati fitur bot ini!\n\n!daftar nama|umur\n\n──「 KBOTZ 」──`,
			...
			const ownerNumber = ["6288802377901@s.whatsapp.net"]
			const mod = [ownerNumber,"6288802377901@s.whatsapp.net"]
			const adminbotnumber = ["6288802377901@s.whatsapp.net"]
			const frendsowner = ["6288802377901@s.whatsapp.net"]
			const premium = ["6288802377901@s.whatsapp.net"]
       ...
       //function leveling
			...
			switch(command) {
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
					client.sendMessage(from, '*KBOTZ GROUP*\n\n *Link* : https://chat.whatsapp.com/GfDCAYcThsbLQCIonVZJRd', text, { quoted: mek })
					break
					case 'funmenu':
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
					...
					case 'nobg':
                                        ...
                                        anu1 = await fetchJson(`https://api.vhtear.com/removebgwithurl?link=${teks}&apikey=RAMLANGANS`, {method: 'get'})
			       ...
			       case 'valxlove':
					...
					if (!isUser) return reply(mess.only.userB)
					if (!isPremium) return reply('Anda bukan Member Premium, silakan hubungi owner atau ketik *!daftarvip* untuk membeli akses Premium!' ,text, { quoted: mek })
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Ciee ada yang mau vip')
					...
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
					tod = await getBuffer(`https://upload.wikimedia.org/wikipedia/commons/thumb/c/c2/Gnome-stock_person_bot.svg/1200px-Gnome-stock_person_bot.svg.png`)
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT KBOTZ ✨* 」\n*│+wa.me/6288802377901*╰──────*「 *KBOTZ* 」*────*\n\n*_JIKA INGIN MENJADI ADMIN  KBOTZ_*\n*_Ketik /iklan_*' })
					break
					case 'premiumcek':
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
					sa = await getBuffer(`https://upload.wikimedia.org/wikipedia/commons/thumb/c/c2/Gnome-stock_person_bot.svg/1200px-Gnome-stock_person_bot.svg.png`)
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*╭────*「 *PREMIUM USER👑* 」\n*│+ wa.me/6288802377901*╰──────*「 * KBOTZ* 」*────*\n\n*_JIKA INGIN MENJADI PREMIUM USER KBOTZ_*\n*_Ketik !daftarvip*' })
					break
					case 'cekmod': 
            ...
            break
					...
					if (!isUser) return reply(mess.only.userB)
					if (!isPremium) return reply('Anda bukan Member Premium, silakan hubungi owner atau ketik *!daftarvip* untuk membeli akses Premium!' ,text, { quoted: mek })
					qute1 = await getBuffer(`https://upload.wikimedia.org/wikipedia/commons/thumb/c/c2/Gnome-stock_person_bot.svg/1200px-Gnome-stock_person_bot.svg.png`)
					...
					if (!isUser) return reply(mess.only.userB)
					if (!isPremium) return reply('Anda bukan Member Premium, silakan hubungi owner atau ketik *!daftarvip* untuk membeli akses Premium!' ,text, { quoted: mek })
					qute = await getBuffer(`https://upload.wikimedia.org/wikipedia/commons/thumb/c/c2/Gnome-stock_person_bot.svg/1200px-Gnome-stock_person_bot.svg.png`)
					...
					if (!isUser) return reply(mess.only.userB)
					if (!isPremium) return reply('Anda bukan Member Premium, silakan hubungi owner atau ketik *!daftarvip* untuk membeli akses Premium!' ,text, { quoted: mek })
					qute2 = await getBuffer(`https://upload.wikimedia.org/wikipedia/commons/thumb/c/c2/Gnome-stock_person_bot.svg/1200px-Gnome-stock_person_bot.svg.png`)
					...
					if (!isUser) return reply(mess.only.userB)
					if (!isPremium) return reply('Anda bukan Member Premium, silakan hubungi owner atau ketik *!daftarvip* untuk membeli akses Premium!' ,text, { quoted: mek })
					qute3 = await getBuffer(`https://upload.wikimedia.org/wikipedia/commons/thumb/c/c2/Gnome-stock_person_bot.svg/1200px-Gnome-stock_person_bot.svg.png`)
					...
					if (!isUser) return reply(mess.only.userB)
					if (!isPremium) return reply('Anda bukan Member Premium, silakan hubungi owner atau ketik *!daftarvip* untuk membeli akses Premium!' ,text, { quoted: mek })
					qute4 = await getBuffer(`https://upload.wikimedia.org/wikipedia/commons/thumb/c/c2/Gnome-stock_person_bot.svg/1200px-Gnome-stock_person_bot.svg.png`)
					...
					if (!isUser) return reply(mess.only.userB)
					if (!isPremium) return reply('Anda bukan Member Premium, silakan hubungi owner atau ketik *!daftarvip* untuk membeli akses Premium!' ,text, { quoted: mek })
					qute5 = await getBuffer(`https://upload.wikimedia.org/wikipedia/commons/thumb/c/c2/Gnome-stock_person_bot.svg/1200px-Gnome-stock_person_bot.svg.png`)
					...
					if (!isUser) return reply(mess.only.userB)
					if (!isPremium) return reply('Anda bukan Member Premium, silakan hubungi owner atau ketik *!daftarvip* untuk membeli akses Premium!' ,text, { quoted: mek })
					qute6 = await getBuffer(`https://upload.wikimedia.org/wikipedia/commons/thumb/c/c2/Gnome-stock_person_bot.svg/1200px-Gnome-stock_person_bot.svg.png`)
				   ...
				   if (args.length < 1) return reply('Apa yang mau dicari kak?')
					goo = body.slice(7)
					anu = await fetchJson(`https://api.vhtear.com/googleimg?query=${goo}&apikey=RAMLANGANS`, {method: 'get'})
					reply(mess.wait)
				   ...
				   if (!isUser) return reply(mess.only.userB)
					...
					if (!isUser) return reply(mess.only.userB)
					if (!isPremium) return reply('Anda bukan Member Premium, silakan hubungi owner atau ketik *!daftarvip* untuk membeli akses Premium!' ,text, { quoted: mek })
					qute7 = await getBuffer(`https://upload.wikimedia.org/wikipedia/commons/thumb/c/c2/Gnome-stock_person_bot.svg/1200px-Gnome-stock_person_bot.svg.png`)
					...
					if (!isUser) return reply(mess.only.userB)
					if (!isPremium) return reply('Anda bukan Member Premium, silakan hubungi owner atau ketik *!daftarvip* untuk membeli akses Premium!' ,text, { quoted: mek })
					qute8 = await getBuffer(`https://upload.wikimedia.org/wikipedia/commons/thumb/c/c2/Gnome-stock_person_bot.svg/1200px-Gnome-stock_person_bot.svg.png`)
					...
					if (!isUser) return reply(mess.only.userB)
					if (!isPremium) return reply('Anda bukan Member Premium, silakan hubungi owner atau ketik *!daftarvip* untuk membeli akses Premium!' ,text, { quoted: mek })
					qute9 = await getBuffer(`https://upload.wikimedia.org/wikipedia/commons/thumb/c/c2/Gnome-stock_person_bot.svg/1200px-Gnome-stock_person_bot.svg.png`)
					...
					if (!isUser) return reply(mess.only.userB)
					if (!isPremium) return reply('Anda bukan Member Premium, silakan hubungi owner atau ketik *!daftarvip* untuk membeli akses Premium!' ,text, { quoted: mek })
					qute10 = await getBuffer(`https://upload.wikimedia.org/wikipedia/commons/thumb/c/c2/Gnome-stock_person_bot.svg/1200px-Gnome-stock_person_bot.svg.png`)
					...
					if (!isUser) return reply(mess.only.userB)
					if (!isPremium) return reply('Anda bukan Member Premium, silakan hubungi owner atau ketik *!daftarvip* untuk membeli akses Premium!' ,text, { quoted: mek })
					qute12 = await getBuffer(`https://upload.wikimedia.org/wikipedia/commons/thumb/c/c2/Gnome-stock_person_bot.svg/1200px-Gnome-stock_person_bot.svg.png`)
					...
					if (!isUser) return reply(mess.only.userB)
					if (!isPremium) return reply('Anda bukan Member Premium, silakan hubungi owner atau ketik *!daftarvip* untuk membeli akses Premium!' ,text, { quoted: mek })
					qute11 = await getBuffer(`https://upload.wikimedia.org/wikipedia/commons/thumb/c/c2/Gnome-stock_person_bot.svg/1200px-Gnome-stock_person_bot.svg.png`)
					...
					if (!isUser) return reply(mess.only.userB)
					if (!isPremium) return reply('Anda bukan Member Premium, silakan hubungi owner atau ketik *!daftarvip* untuk membeli akses Premium!' ,text, { quoted: mek })
					qute13 = await getBuffer(`https://upload.wikimedia.org/wikipedia/commons/thumb/c/c2/Gnome-stock_person_bot.svg/1200px-Gnome-stock_person_bot.svg.png`)
					...
					if (!isUser) return reply(mess.only.userB)
					if (!isPremium) return reply('Anda bukan Member Premium, silakan hubungi owner atau ketik *!daftarvip* untuk membeli akses Premium!' ,text, { quoted: mek })
					qute14 = await getBuffer(`https://upload.wikimedia.org/wikipedia/commons/thumb/c/c2/Gnome-stock_person_bot.svg/1200px-Gnome-stock_person_bot.svg.png`)
					...
					if (!isUser) return reply(mess.only.userB)
					if (!isPremium) return reply('Anda bukan Member Premium, silakan hubungi owner atau ketik *!daftarvip* untuk membeli akses Premium!' ,text, { quoted: mek })
					qute15 = await getBuffer(`https://upload.wikimedia.org/wikipedia/commons/thumb/c/c2/Gnome-stock_person_bot.svg/1200px-Gnome-stock_person_bot.svg.png`)
					...
					if (!isUser) return reply(mess.only.userB)
					if (!isPremium) return reply('Anda bukan Member Premium, silakan hubungi owner atau ketik *!daftarvip* untuk membeli akses Premium!' ,text, { quoted: mek })
					qute16 = await getBuffer(`https://upload.wikimedia.org/wikipedia/commons/thumb/c/c2/Gnome-stock_person_bot.svg/1200px-Gnome-stock_person_bot.svg.png`)
					...
					if (!isUser) return reply(mess.only.userB)
					if (!isPremium) return reply('Anda bukan Member Premium, silakan hubungi owner atau ketik *!daftarvip* untuk membeli akses Premium!' ,text, { quoted: mek })
					qute17 = await getBuffer(`https://upload.wikimedia.org/wikipedia/commons/thumb/c/c2/Gnome-stock_person_bot.svg/1200px-Gnome-stock_person_bot.svg.png`)
					...
					if (!isUser) return reply(mess.only.userB)
					if (!isPremium) return reply('Anda bukan Member Premium, silakan hubungi owner atau ketik *!daftarvip* untuk membeli akses Premium!' ,text, { quoted: mek })
					qute18 = await getBuffer(`https://upload.wikimedia.org/wikipedia/commons/thumb/c/c2/Gnome-stock_person_bot.svg/1200px-Gnome-stock_person_bot.svg.png`)
					...
					if (!isUser) return reply(mess.only.userB)
					if (!isPremium) return reply('Anda bukan Member Premium, silakan hubungi owner atau ketik *!daftarvip* untuk membeli akses Premium!' ,text, { quoted: mek })
					qute19 = await getBuffer(`https://upload.wikimedia.org/wikipedia/commons/thumb/c/c2/Gnome-stock_person_bot.svg/1200px-Gnome-stock_person_bot.svg.png`)
					...
					if (!isUser) return reply(mess.only.userB)
					if (!isPremium) return reply('Anda bukan Member Premium, silakan hubungi owner atau ketik *!daftarvip* untuk membeli akses Premium!' ,text, { quoted: mek })
					qute20 = await getBuffer(`https://upload.wikimedia.org/wikipedia/commons/thumb/c/c2/Gnome-stock_person_bot.svg/1200px-Gnome-stock_person_bot.svg.png`)
					...
					if (!isUser) return reply(mess.only.userB)
					if (!isPremium) return reply('Anda bukan Member Premium, silakan hubungi owner atau ketik *!daftarvip* untuk membeli akses Premium!' ,text, { quoted: mek })
					qute21 = await getBuffer(`https://upload.wikimedia.org/wikipedia/commons/thumb/c/c2/Gnome-stock_person_bot.svg/1200px-Gnome-stock_person_bot.svg.png`)
					...
					if (!isUser) return reply(mess.only.userB)
					if (!isPremium) return reply('Anda bukan Member Premium, silakan hubungi owner atau ketik *!daftarvip* untuk membeli akses Premium!' ,text, { quoted: mek })
					qute22 = await getBuffer(`https://upload.wikimedia.org/wikipedia/commons/thumb/c/c2/Gnome-stock_person_bot.svg/1200px-Gnome-stock_person_bot.svg.png`)
					...
					if (!isUser) return reply(mess.only.userB)
					if (!isPremium) return reply('Anda bukan Member Premium, silakan hubungi owner atau ketik *!daftarvip* untuk membeli akses Premium!' ,text, { quoted: mek })
					qute23 = await getBuffer(`https://upload.wikimedia.org/wikipedia/commons/thumb/c/c2/Gnome-stock_person_bot.svg/1200px-Gnome-stock_person_bot.svg.png`)
					...
					if (!isUser) return reply(mess.only.userB)
					if (!isPremium) return reply('Anda bukan Member Premium, silakan hubungi owner atau ketik *!daftarvip* untuk membeli akses Premium!' ,text, { quoted: mek })
					qute24 = await getBuffer(`https://upload.wikimedia.org/wikipedia/commons/thumb/c/c2/Gnome-stock_person_bot.svg/1200px-Gnome-stock_person_bot.svg.png`)
		...
		case 'iri':
         ...
         if (!isPremium) return reply(mess.only.premium)
          if (isBanned) return reply(mess.only.benned)   
                ...
                data = await fetchJson(`https://api.vhtear.com/ytmp3?query=${query}&apikey=RAMLANGANS`, {method: 'get'})
 ...
 if (!isOwner) return reply(mess.only.ownerB)
				...
				if (!isUser) return reply(mess.only.userB)
					...
					teks = body.slice(11)
					if (teks.length > 15) return reply('Teksnya kepanjangan, maksimal 8 karakter')
					reply(mess.wait)
					bufasfer = await getBuffer(`https://api.vhtear.com/galaxytext?text=${teks}&apikey=RAMLANGANS`, {method: 'get'})
            ...
            break
				...
				if (!isUser) return reply(mess.only.userB)
					...
					teks = body.slice(6)
					reply(mess.wait)
					loog = await getBuffer(`https://api.vhtear.com/gamelogo?text=${teks}&apikey=RAMLANGANS`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					...
					anu = await fetchJson(`https://api.vhtear.com/primbonjodoh?nama=${gbl11}&pasangan=${gbl21}&apikey=RAMLANGANS`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					...
					anu = await fetchJson(`https://api.vhtear.com/harijadian?tgl=${gbl16}&bln=${gbl26}&thn=${gbl36}&apikey=RAMLANGANS`, {method: 'get'})
					reply(anu.result.hasil)
					break
                case 'tahta':
                if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					...
					reply(mess.wait)
					bufferrty = await getBuffer(`https://api.vhtear.com/hartatahta?text=${teks}&apikey=RAMLANGANS`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					...
					pinte = body.slice(11)
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=${pinte}&apikey=RAMLANGANS`, {method: 'get'})
					reply(mess.wait)
...
if (isBanned) return reply(mess.only.benned)    
				...
				if (!isUser) return reply(mess.only.userB)
					...
					reply(mess.wait)
					bufferxcz = await getBuffer(`https://api.vhtear.com/lovemessagetext?text=${love}&apikey=RAMLANGANS`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if (love.length > 12) return reply('Teksnya kepanjangan, maksimal 9 karakter')
					reply(mess.wait)
					bufferxcz = await getBuffer(`https://api.vhtear.com/metal_maker?text=${love}&apikey=RAMLANGANS`, {method: 'get'})
					client.sendMessage(from, bufferxcz, image, {quoted: mek, caption: ' '+love})
					break
				case 'apiteks':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					...
					if (love.length > 12) return reply('Teksnya kepanjangan, maksimal 9 karakter')
					reply(mess.wait)
					bufferxcz = await getBuffer(`https://api.vhtear.com/fire_maker?text=${love}&apikey=RAMLANGANS`, {method: 'get'})
					client.sendMessage(from, bufferxcz, image, {quoted: mek, caption: ' '+love})
					break
				case 'ffbaner':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					...
					if (love.length > 12) return reply('Teksnya kepanjangan, maksimal 9 karakter')
					reply(mess.wait)
					bufferxcz = await getBuffer(`https://api.vhtear.com/bannerff?title=${love}&text=Kanetav&apikey=RAMLANGANS`, {method: 'get'})
					client.sendMessage(from, bufferxcz, image, {quoted: mek, caption: ' '+love})
					break
					case 'goldbutton':
				if (isBanned) return reply(mess.only.benned)    
          ...
          if (isBanned) return reply(mess.only.benned)    
				...
				if (!isUser) return reply(mess.only.userB)
					...
					reply(mess.wait)
					bufferw = await getBuffer(`https://api.vhtear.com/thundertext?text=${thun}&apikey=RAMLANGANS`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
                      ...
                      gl1 = gdh.split("|")[0];
                      gl2 = gdh.split("|")[1];
                      buffere = await getBuffer(`https://api.vhtear.com/silktext?text=${gl1}&text2=${gl2}&apikey=RAMLANGANS`, {method: 'get'})
                      reply(mess.wait)
				...
				if (!isUser) return reply(mess.only.userB)
					...
					part = body.slice(7)
					reply(mess.wait)
					bufferu = await getBuffer(`https://api.vhtear.com/partytext?text=${part}&apikey=RAMLANGANS`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					...
					part = body.slice(7)
					reply(mess.wait)
					bufferu = await getBuffer(`https://api.vhtear.com/logoml?hero=miya&text=${part}&apikey=RAMLANGANS`, {method: 'get'})
					client.sendMessage(from, bufferu, image, {caption: 'Nih kak', quoted: mek})
					break
				case 'shadow':
				if (isBanned) return reply(mess.only.benned)    
				...
				if (!isUser) return reply(mess.only.userB)
					...
					reply(mess.wait)
					bufferi = await getBuffer(`https://api.vhtear.com/romancetext?text=${tels5}&apikey=RAMLANGANS`, {method: 'get'})
	...
	if (isBanned) return reply(mess.only.benned)    
				...
				if (!isUser) return reply(mess.only.userB)
					...
					truteh = await getBuffer(`https://i.ibb.co/tZWS1t3/KBOTZTH.jpg`)
				...
				if (!isUser) return reply(mess.only.userB)
					...
					tod = await getBuffer(`https://i.ibb.co/1RjvqKc/KBOTZDE.jpg`)
...
if (isBanned) return reply(mess.only.benned)    
               ...
               client.reply(from, `Waalaikumsalam wr.wb${pushname}:)`)
				...
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					client.sendMessage(from, '*indosat :* _6285759540313_\n', text, { quoted: mek })
					break
                case 'ttp':
                if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
				...
				if (!isUser) return reply(mess.only.userB)
                    ...
                    client.sendMessage('6288802377901@s.whatsapp.net', options, text, {quoted: mek})
			...
			if (isBanned) return reply(mess.only.benned)    
				...
				if (!isUser) return reply(mess.only.userB)
					...
					teksj = body.slice(7)
					reply(mess.wait)
					anwu = await fetchJson(`https://api.vhtear.com/walpaper?query=${teksj}&apikey=RAMLANGANS`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
						...
						res = await fetchJson(`https://api.vhtear.com/randomloli&apikey=RAMLANGANS`, {method: 'get'})
			...
			if (isBanned) return reply(mess.only.benned)    
				...
				if (!isUser) return reply(mess.only.userB)
					...
					reply(mess.wait)
					anu = await fetchJson(`https://api.vhtear.com/randomwibu&apikey=RAMLANGANS`, {method: 'get'})
					if (anu.error) return reply(anu.error)
				...
				if (!isUser) return reply(mess.only.userB)
					...
					reply(mess.wait)
					anu = await fetchJson(`https://api.vhtear.com/nhentaipdfdownload?query=287167&apikey=RAMLANGANS`, {method: 'get'})
					if (anu.error) return reply(anu.error)
				...
				if (!isUser) return reply(mess.only.userB)
					reply(mess.wait)
					anu = await fetchJson(`https://api.vhtear.com/randomcat?apikey=RAMLANGANS`, {method: 'get'})
					if (anu.error) return reply(anu.error)
				...
				if (!isUser) return reply(mess.only.userB)
					reply(mess.wait)
					anu = await fetchJson(`https://api.vhtear.com/mlherolist?apikey=RAMLANGANS`, {method: 'get'})
 ...
 if (isBanned) return reply(mess.only.benned)    
                ...
                client.sendMessage(hasil, image, mek, '\`\`\`KBOTZJOKES\`\`\`')
				break
					case 'kata':
			...
			if (isBanned) return reply(mess.only.benned)    
               ...
               if (!isUser) return reply(mess.only.userB)
                ...
                data = await fetchJson(`https://api.vhtear.com/music?query=${body.slice(6)}&apikey=RAMLANGANS`, {method: 'get'})
          ...
          reply('Wait..')
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if (args.length < 1) return reply('teks nya mana om')
					kj = body.slice(12)
					anu = await fetchJson(`https://api.vhtear.com/nomerhoki?no=${kj}&apikey=RAMLANGANS`, {method: 'get'})
					reply(anu.result.hasil)
					break
				case 'textscreen':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					...
					if (args.length < 1) return reply('teks nya mana om')
					telsjk = body.slice(9)
					anu = await fetchJson(`https://api.vhtear.com/textscreen?query=${telsjk}&apikey=RAMLANGANS`, {method: 'get'})
			...
			if (isBanned) return reply(mess.only.benned)    
               ...
               if (data.error) return reply(data.error)
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isGroup) return reply(mess.only.group)
					reply(mess.asik)
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=hewanimutlucu&apikey=RAMLANGANS`, {method: 'get'})
...
if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					anu = await fetchJson(`https://api.vhtear.com/tebakgambar&apikey=RAMLANGANS`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					...
					anu = await fetchJson(`https://api.vhtear.com/funkuis&apikey=RAMLANGANS`, {method: 'get'})
					setTimeout( () => {
				...
				if (!isUser) return reply(mess.only.userB)
					...
					anu = await fetchJson(`https://api.vhtear.com/family100&apikey=RAMLANGANS`, {method: 'get'})
					setTimeout( () => {
					client.sendMessage(from, '*➸ Jawaban :* '+anu.result.jawaban, text, {quoted: mek}) // ur cods
					}, 30000) // 1000 = 1s,
					setTimeout( () => {
...
if (isBanned) return reply(mess.only.benned)    
				...
				if (!isUser) return reply(mess.only.userB)
					...
					anu = await fetchJson(`https://api.vhtear.com/branly?query=${body.slice(9)}&apikey=RAMLANGANS`, {method: 'get'})
 ...
 if (isBanned) return reply(mess.only.benned)    
				...
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=elang&apikey=RAMLANGANS`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					...
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=naruto&apikey=RAMLANGANS`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isAnime) return reply('❌ *Harus Mengaktifkan Mode Anime* ❌')
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=minato&apikey=RAMLANGANS`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isAnime) return reply('❌ *Harus Mengaktifkan Mode Anime* ❌')
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=boruto&apikey=RAMLANGANS`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isAnime) return reply('❌ *Harus Mengaktifkan Mode Anime* ❌')
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=hinata&apikey=RAMLANGANS`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isAnime) return reply('❌ *Harus Mengaktifkan Mode Anime* ❌')
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=sasuke&apikey=RAMLANGANS`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isAnime) return reply('❌ *Harus Mengaktifkan Mode Anime* ❌')
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=sakura&apikey=RAMLANGANS`, {method: 'get'})
				...
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=unta&apikey=RAMLANGANS`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isAnime) return reply('❌ *Harus Mengaktifkan Mode Anime* ❌')
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=kaneki&apikey=RAMLANGANS`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isAnime) return reply('❌ *Harus Mengaktifkan Mode Anime* ❌')
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=hewanimut&apikey=RAMLANGANS`, {method: 'get'})
					var kan = JSON.parse(JSON.stringify(anu.result));
					var eki =  kan[Math.floor(Math.random() * kan.length)];
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isAnime) return reply('❌ *Harus Mengaktifkan Mode Anime* ❌')
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=ToukaKirishima&apikey=RAMLANGANS`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isAnime) return reply('❌ *Harus Mengaktifkan Mode Anime* ❌')
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=RizeKamishiro&apikey=RAMLANGANS`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isAnime) return reply('❌ *Harus Mengaktifkan Mode Anime* ❌')
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=akiramado&apikey=RAMLANGANS`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isAnime) return reply('❌ *Harus Mengaktifkan Mode Anime* ❌')
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=itori&apikey=RAMLANGANS`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isAnime) return reply('❌ *Harus Mengaktifkan Mode Anime* ❌')
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=kurumitokisakikawai&apikey=RAMLANGANS`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isAnime) return reply('❌ *Harus Mengaktifkan Mode Anime* ❌')
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=Nakanomiku&apikey=RAMLANGANS`, {method: 'get'})
					var mi = JSON.parse(JSON.stringify(anu.result));
					var ku =  mi[Math.floor(Math.random() * mi.length)];
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isAnime) return reply('❌ *Harus Mengaktifkan Mode Anime* ❌')
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=remrezero&apikey=RAMLANGANS`, {method: 'get'})
					var mi = JSON.parse(JSON.stringify(anu.result));
					var ku =  mi[Math.floor(Math.random() * mi.length)];
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isAnime) return reply('❌ *Harus Mengaktifkan Mode Anime* ❌')
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=ramrezero&apikey=RAMLANGANS`, {method: 'get'})
					var mi = JSON.parse(JSON.stringify(anu.result));
					var ku =  mi[Math.floor(Math.random() * mi.length)];
				...
				if (!isUser) return reply(mess.only.userB)
					...
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=bokep&apikey=RAMLANGANS`, {method: 'get'})
					var mi = JSON.parse(JSON.stringify(anu.result));
					var ku =  mi[Math.floor(Math.random() * mi.length)];
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isGroup) return reply(mess.only.group)
					reply(mess.asik)
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=cewekcantik&apikey=RAMLANGANS`, {method: 'get'})
					var mi = JSON.parse(JSON.stringify(anu.result));
					var ku =  mi[Math.floor(Math.random() * mi.length)];
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isGroup) return reply(mess.only.group)
					reply(mess.asik)
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=cowokganteng&apikey=RAMLANGANS`, {method: 'get'})
					var mi = JSON.parse(JSON.stringify(anu.result));
					var ku =  mi[Math.floor(Math.random() * mi.length)];
			...
			if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					...
					setTimeout( () => {
					 client.sendMessage(from, 'siap siap ya KBOTZ Kick nih', text, { quoted: mek }) // ur cods
					}, 0) // 1000 = 10s,
					}
					break
				case 'aguse':
				...
				if (!isUser) return reply(mess.only.userB)
					if (args.length < 1) return reply(mess.search)
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=hewan&apikey=RAMLANGANS`, {method: 'get'})
					reply(mess.wait)
					var pin = JSON.parse(JSON.stringify(anu.result));
					var trest =  pin[Math.floor(Math.random() * pin.length)];
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isGroup) return reply(mess.only.group)
					reply(mess.asik)
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=lolikawai&apikey=RAMLANGANS`, {method: 'get'})
					var mi = JSON.parse(JSON.stringify(anu.result));
					var ku =  mi[Math.floor(Math.random() * mi.length)];
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isGroup) return reply(mess.only.group)
					reply(mess.asik)
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=waifukawai&apikey=RAMLANGANS`, {method: 'get'})
					var mi = JSON.parse(JSON.stringify(anu.result));
					var ku =  mi[Math.floor(Math.random() * mi.length)];
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isGroup) return reply(mess.only.group)
					reply(mess.asik)
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=nekoanimekawai&apikey=RAMLANGANS`, {method: 'get'})
					var mi = JSON.parse(JSON.stringify(anu.result));
					var ku =  mi[Math.floor(Math.random() * mi.length)];
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isNsfw) return reply('❌ *FALSE* ❌')
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=animehentai&apikey=RAMLANGANS`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isAnime) return reply('❌ *Harus Mengaktifkan Mode Anime* ❌')
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=loli&apikey=RAMLANGANS`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if (args.length < 1) return reply(mess.search)
					pinte = body.slice(11)
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=${pin}&apikey=RAMLANGANS`, {method: 'get'})
					reply(mess.wait)
					var pin = JSON.parse(JSON.stringify(anu.result));
					var trest =  pin[Math.floor(Math.random() * pin.length)];
					pine = await getBuffer(trest)
				...
				if (!isUser) return reply(mess.only.userB)
					...
					reply(mess.wait)
					anu = await fetchJson(`https://api.vhtear.com/tiktokdl?link=https://vt.tiktok.com/yqyjPX/&apikey=RAMLANGANS`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					buffer = await getBuffer(anu.result)
			...
			case 'woodenlogo':
				...
				if (!isUser) return reply(mess.only.userB)
					...
					tulis = body.slice(6)
					reply(mess.wait)
					buffer4 = await getBuffer(`https://api.vhtear.com/write?text=${tulis}&apikey=RAMLANGANS`, {method: 'get'})
			...
			if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
                    ...
                    teks = `${body.slice(8)}`
                    if (teks.length > 10) return client.sendMessage(from, 'Teksnya kepanjangan, Maksimal 10 kalimat', text, {quoted: mek})
                    buffer6 = await getBuffer(`https://api.vhtear.com/lovemessagetext?text=${teks}&apikey=RAMLANGANS`, {method: 'get'})
                    client.sendMessage(from, buffer6, image, {quoted: mek, caption: `${teks}`})
			     	break
			     case 'airtext':
			if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
                    ...
                    teks = `${body.slice(8)}`
                    if (teks.length > 10) return client.sendMessage(from, 'Teksnya kepanjangan, Maksimal 10 kalimat', text, {quoted: mek})
                    buffer6 = await getBuffer(`https://api.vhtear.com/water_maker?text=${teks}&apikey=RAMLANGANS`, {method: 'get'})
                    client.sendMessage(from, buffer6, image, {quoted: mek, caption: `${teks}`})
			     	break
			     case 'kuncitext':
			if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
                    ...
                    teks = `${body.slice(8)}`
                    if (teks.length > 10) return client.sendMessage(from, 'Teksnya kepanjangan, Maksimal 10 kalimat', text, {quoted: mek})
                    buffer6 = await getBuffer(`https://api.vhtear.com/padlock?text1=${teks}&text2=${teks}&apikey=RAMLANGANS`, {method: 'get'})
                    client.sendMessage(from, buffer6, image, {quoted: mek, caption: `${teks}`})
			     	break
			    case 'shorturl':
			if (isBanned) return reply(mess.only.benned)    
			...
			if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					...
					ige = body.slice(9)
					reply(mess.wait)
					anu = await fetchJson(`https://api.vhtear.com/igprofile?query=${ige}&apikey=RAMLANGANS`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					...
					ige = body.slice(9)
					reply(mess.wait)
					anu = await fetchJson(`https://api.vhtear.com/infomobil?merk=${ige}&apikey=RAMLANGANS`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					...
					ft1 = body.slice(11)
					reply(mess.wait)
					anu = await fetchJson(`https://api.vhtear.com/infomotor?merk=${ft1}&apikey=RAMLANGANS`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					kuji = body.slice(7)
					reply(mess.wait)
					anu = await getBuffer(`https://api.vhtear.com/playstore?query={kuji}&apikey=RAMLANGANS`, {method: 'get'})
 ...
 if (!isOwner) return reply(mess.only.ownerB)
			...
			if (isBanned) return reply(mess.only.benned)    
				...
				if (!isUser) return reply(mess.only.userB)
					...
					reply(mess.wait)
					anu = await fetchJson(`https://mhankbarbars.herokuapp.com/api/url2image?tipe=${args[0]}&url=${args[1]}&apikey=RAMLANGANS`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					...
					reply(mess.wait)
					anu = await fetchJson(`https://api.vhtear.com/logoff?hero=alok&text=${tels}&apikey=RAMLANGANS`, {method: 'get'})
					buffer = await getBuffer(anu.result)
					 client.sendMessage(from, buffer, image, {quoted: mek})
					break
				case 'tstiker':
			...
			if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					...
					mentions('╔══✪〘 Mention All 〙✪══'+teks+'╚═〘  KBOTZ 〙', members_id, true)
					break
				case 'clearall':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ * KBOTZ Broadcast* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
	...
	.then(() => {client.sendMessage(from, options, text)})
				...
				//frendowner
				case `addadmin`:
                   if (!isOwner) return reply('Perintah ini hanya bisa di gunakan oleh Owner KBOTZ!', id)
                   for (let i = 0; i < mentionedJidList.length; i++) {
                   adminNumber.push(mentionedJidList[i])
                   fs.writeFileSync('./lib/database/admin.json', JSON.stringify(adminNumber))
                   reply('Success Menambahkan Admin KBOTZ!')
				   }
                   break
				case 'wait':
				if (isBanned) return reply(mess.only.benned)    
